...
const okshadow = function (el, options) {
    var base = this;
    base.el = el;
    el.setAttribute('okshadow', base);

    var active = true;

    base.init = function () {
        base.options = { ...default_options, ...options };
        base.build();
    };
    ...
    base.start = function () {
        if (detectMobile() && ('DeviceOrientationEvent' in window)) {
            ...
            base.mousemove({ pageX: window.innerWidth / 2, pageY: window.innerHeight / 2 });
        }
    ...
    base.mousemove = function (e) {
        var rect = base.el.getBoundingClientRect();
        var offset = {
            ...
            x = e.pageX,
            y = e.pageY,
            cy = offset.top + base.el.clientHeight / 2,
            cx = offset.left + base.el.clientWidth / 2,
            dx = (cx - x),
            dy = (cy - y),
    ...
    base.update = function (sx, sy, fuzz) {
        ...
        for (var i in base.browsers) {
            ...
            base.el.style[base.browsers[i] + prop] = val
        }
    };
